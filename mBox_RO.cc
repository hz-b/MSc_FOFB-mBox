...
RFM2G_UINT32   DAC_Buffer2[DAC_BUFFER_SIZE];
...
template <class T>
void prepareField(T& field, unsigned long pos, unsigned long dim1, unsigned long dim2) {
...
template <>
void prepareField(double *& field, unsigned long pos, unsigned long dim1, unsigned long dim2) {
...
template <>
void prepareField(vec& field, unsigned long pos, unsigned long dim1, unsigned long dim2) {
...
template <>
void prepareField(mat& field, unsigned long pos, unsigned long dim1, unsigned long dim2) {
    RFM2G_UINT32 threshold = 0, data_size=dim1*dim2*8;
    /* see if DMA threshold and buffer are intialized */
    RFM2gGetDMAThreshold( RFM_Handle, &threshold );

    cout << "pos="<<setw(12) << pos << endl;
    ...
    RFM2gRead(RFM_Handle,pos,(void*)pDmaMemory, data_size);
    dumpMemory(pDmaMemory,8);
    dumpMemory(pDmaMemory+8,8);
    dumpMemory(pDmaMemory+16,8);

    field=mat((const double *)pDmaMemory,dim1,dim2);
    }
    cout << "m5" << field(0,0) << " " << field(0,1) << endl;
...
void readStruct(const char * structname, T & field, char tartype = 0) {
    ...
    if (tartype==readStructtype_mat) cout << "pos="<<setw(12) << pos << endl;
    RFM2gRead(RFM_Handle,pos,&header,8); // 4 * 16-Bit = 8
    //cout << "2" << endl;
    pos += 8;

    unsigned long namesize  = header[0];
    unsigned long datasize1 = header[1];
    unsigned long datasize2 = header[2];
    ...
    cout << "3 namesize: " << namesize << " "<< name << endl;
...
void calcSmat() {
    ...
    if (IvecX > SmatX.n_rows) {
      cout << "IVecX > SmatX.n_rows: Setting IvecX = SmatX.n_rows" << SmatX.n_rows << endl;
    IvecX = SmatX.n_rows;
    }
...
unsigned char make_cor() {
    ...
    RFM2G_UINT32    rfm2gMemNumber = status.loopPos;
...
int quick_check() {
   /* --- start timer --- */
    ...
    cout<<"loopPos "<<status.loopPos<<endl;
    ...
    cout << "DACBuffer" << endl;
    if ((DAC_Buffer[0] != 0) && (DAC_Buffer2[0] != 0)){ 
      for (int i=0; i < 48; i++) {
        int corPos = DAC_WaveIndexX[i]-1;
        double v1 = (DAC_Buffer[corPos] -halfDigits)/scaleDigitsX(0);
        double v2 = (DAC_Buffer2[corPos]-halfDigits)/scaleDigitsX(0);
        cout << std::setw( 11 ) << std::setprecision( 6 ) << (v2-v1) << endl;
      }
    } else {
      cout << "Buffers unfilled" << endl;
    }
    cout << "dCORx:" << endl ; //<< std::scientific << std::setw(12) << std::right << std::setprecision( 6 ) << dCORxPID  << endl;
    for (int i=0; i < 48; i++) {
	cout << std::setw( 11 ) << std::setprecision( 6 ) << dCORxPID(i) << endl;
    }
